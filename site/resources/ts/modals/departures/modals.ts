...
export const setupModalEventListenersDepartures = (): void => {
	...
	document
		.querySelectorAll<HTMLButtonElement>('[data-bs-dismiss="modal"]')
		.forEach((button) => {
...
const handleEditDepartureModal = (
	...
	if (editUrlTemplate) {
		editForm.action = editUrlTemplate.replace(
			":id",
	...
	checkboxes.forEach((checkbox) => {
		checkbox.checked = observers.includes(
...
const handleDetailsDepartureModal = (
	...
	const detailsBoatName =
		modal.querySelector<HTMLElement>("#details_boat_name");
	const detailsTransectName = modal.querySelector<HTMLElement>(
		"#details_transect_name",
	);
	const detailsDate = modal.querySelector<HTMLElement>("#details_date");
	const detailsObservers =
		modal.querySelector<HTMLUListElement>("#details_observers");
...
const handleDeleteDepartureModal = (
	modal: HTMLElement,
	departureId: string,
	departureName: string,
): void => {
	const deleteForm = modal.querySelector<HTMLFormElement>("form");
	const textDepartureName = modal.querySelector<HTMLElement>(
		"#deleteDepartureName",
	);
	...
	if (actionTemplate) {
		deleteForm.action = actionTemplate.replace(
			":id",
			departureId.toString(),
		);
	} else {
		console.error(
			"Falta la plantilla de URL de eliminación en el formulario",
		);
		return;
	}
